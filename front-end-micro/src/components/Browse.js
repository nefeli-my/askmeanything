...
const Browse = () => {
  ...
  useEffect(() => {
    ...
    fetch(process.env.REACT_APP_QNA + 'getquestions/0',
        {
          method: 'GET',
  ...
  function getQuestions(e) {
    ...
    if (!author && !keyword && !startDate) {
      ...
      fetch(process.env.REACT_APP_QNA + 'getquestions/' + offset,
          {
            method: 'GET',
            headers: {"Content-Type": "application/json", "Authorization": 'Bearer ' + JSON.parse(token)}
          })
    ...
    // filtered search
      ...
      //format url
      let url = process.env.REACT_APP_QNA + 'getquestions/filters/?'
      if (author)
