...
const MyStatistics = () => {
  ...
  useEffect(() => {
    ...
    setLoading3(true);
    setLoading4(true);
    fetch(process.env.REACT_APP_STATISTICS_URL + 'user/questions/' + Intl.DateTimeFormat().resolvedOptions().timeZone.replace('/','%2F'),
    {
      method: 'GET',
    ...
    // and user's data), when component is mounted
    fetch(process.env.REACT_APP_STATISTICS_URL + 'user/answers/' + Intl.DateTimeFormat().resolvedOptions().timeZone.replace('/','%2F'),
    {
      method: 'GET',
      headers: { "Content-Type": "application/json", "Authorization": 'Bearer '+ JSON.parse(token) }
    })
    ...
    // limit: 20
    fetch(process.env.REACT_APP_STATISTICS_URL + 'user/keywords',
    {
      method: 'GET',
      headers: { "Content-Type": "application/json", "Authorization": 'Bearer '+ JSON.parse(token) }
    })
